...
public class BedrijfControllerTest extends EasyMockSupport {
    ...
    public void testLijst() {
        ...
        expect(gebruikerService.lees(id)).andReturn(relatie);
        expect(bedrijfService.alleBedrijvenBijRelatie(relatie)).andReturn(bedrijven);
        expect(bedrijfMapper.mapAllNaarJson(bedrijvenSet)).andReturn(jsonBedrijven);

        replayAll();

        controller.lijst(id.toString());
    }
